...
no_parallism_cimg: pre-build
	$(GCC) -x c++ src/no_parallism.cpp src/main_cuda.cpp -lpng -ljpeg -o bin/image_modifier_no_parallism

cuda: pre-build
	$(NVCC) -arch=$(CUDA_ARCH) -x cu src/cuda.cu src/main_cuda.cpp -lpng -ljpeg -o bin/image_modifier_cuda
